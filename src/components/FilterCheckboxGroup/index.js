...
const FilterCheckboxGroup = ({ title, options, selectedFilters, onChange }) => {
            ...
            <div className="flex flex-col gap-[10px]">
                {visibleOptions.map(({ id, label }, index) => (
                    <label key={index} className="inline-flex items-center cursor-pointer">
                        <input type="checkbox" className="form-checkbox hidden h-5 w-5 rounded border-gray-300 focus:ring-blue-500" checked={selectedFilters[id] || false} onChange={() => onChange(id)} />
                        <div className="h-5 w-5">{selectedFilters[id] ? <BsCheckSquareFill size={18} className="shadow-md text-info_main" /> : <div className="h-[18px] w-[18px] border border-black/50 rounded-[3px] shadow-md"></div>}</div>
